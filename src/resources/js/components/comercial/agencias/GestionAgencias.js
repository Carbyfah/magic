...
function GestionAgencias() {
    ...
    const ejecutarCambioEstado = async () => {
        if (!agenciaConfirmacion) return;

        try {
            const nuevoEstado = agenciaConfirmacion.situacion === 1 ? 0 : 1;
            const response = await fetch(`/api/v1/agencias/${agenciaConfirmacion.id}`, {
                method: 'PUT',
                headers: {
        ...
        e('div', {
            ...
            loading ? e('div', {
                ...
                e('div', {
                    ...
                    style: {
                        padding: '1.5rem',
                        borderBottom: index < agenciasFiltradas.length - 1 ? '1px solid #f3f4f6' : 'none'
                    }
                }, [
                    e('div', {
                        key: 'agencia-content',
                        ...
                        e('div', {
                            ...
                            e('button', {
                                key: 'btn-estado',
                                onClick: () => abrirModalConfirmacion(agencia, agencia.situacion === 1 ? 'desactivar' : 'activar'),
                                style: {
                                    padding: '0.5rem',
                                    border: 'none',
                                    borderRadius: '6px',
